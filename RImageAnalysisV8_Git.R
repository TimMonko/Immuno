...
for (im in 1:file.number) {
  ...
  plot(dist.tif, 1, main = file.paths[im]) # Second argument allows which layer to show, defaults to 1 if none entered

  px <- dist.tif > 0.1 # > 0.1 if White signal on Black background, < 0.1 if visa versa
  ...
  poly2 <- (poly$x)/1360 # The maximum value for the figures dimensions. This is default to 1360 x 1036 on our microscope.
  poly2t <- t(t(poly2))
  if (flipROI == 1) {
    poly3 <- poly$y/1036
  } else {
    poly3 <- (1036-poly$y)/1036
  }
  #poly3 <- (1036-poly$y)/1036
  poly3t <- t(t(poly3))
  polyc <- matrix(c(poly2t, poly3t), ncol = 2, byrow = FALSE)

  ROI <- rasterizeROI(polyc, c(1360, 1036))
  #addMask(ROI, add = FALSE) # not necessary for code but used to visualize
  ROIcimg <- as.cimg(t(ROI)) # Strangely, needs to be converted to make a "conformable array" although it looks redundant to go mat -> cimg -> mat

  # Convert to matrices/array for doing arithmetic isntead of using the cimg/pixset format
  ROI.mat <- as.matrix(ROIcimg) 
  ROI.flip <- 1 - ROI.mat
  ...
  for (slice in 1:length(dist)) {
    px.mat <- as.matrix(dist.tif[,,slice,])

    combo.mat <- ROI.flip + px.mat # Combine the two matrices into an image for analysis 
    if (flipROI == 1) { 
      combo.crop <- combo.mat[(min.x:max.x), ((1036-min.y):(1036-max.y))] # Maintenance of y-values is flipped across the vertical axis so min + max need subtracted from the number of pixels in height
    } else {
      combo.crop <- combo.mat[(min.x:max.x), (min.y:max.y)]
    }
    combo.mat2 <- combo.crop > 1 # Convert to binary
    combo.cimg <- as.cimg(combo.mat2) # Numeric binary array to cimg
    ...
    centers.my.rel[[slice]]    <- c(centers.my.rel[[slice]], 1-(centers$my/(max.y-min.y)))
    # centers.my.rel[[slice]]    <- c(centers.my.rel[[slice]], 1-(centers$my/max.y)) # This might not need to be 1- if no flip
  }
}
...
## ---- 
WT <- new.env()
load('envWT2.RData', envir = WT)
WTmed <- new.env()
#load('envWTmedial.RData', envir = WTmed)
CKO <- new.env()
load('envCKO2.RData', envir = CKO)
CKOmed <- new.env()
...
#max.length <- max(c(length(unlWT), length(unlCKO))) # This does not seem necessary, for some reason, anymore
#length(unlWT) <- max.length
#length(unlCKO) <- max.length
tib.WT <- tibble(dist = unlWT, geno = 'WT')
tib.CKO <- tibble(dist = unlCKO, geno = 'CKO')
...
# unique.episodes <- group_by(combo.dist.allbins, geno) %>% count(combo.dist.allbins$geno)
# data2 <- merge(x = combo.dist.allbins, y = unique.episodes, by = combo.dist.allbins$geno, all.x = TRUE)
